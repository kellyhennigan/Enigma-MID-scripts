...
for r = 1:numel(roiNames)
    ...
    for f = 1:nFigs
        ...
        % fig title
        figtitle = [strrep(roiName,'_',' ') ' response 2nd derivative to ' stimStr ];
        
        % x and y labels
        xlab = 'TR 2nd derivative';
        ylab = '%\Delta BOLD (2nd derivative)';
        ...
        % filepath, if saving
        savePath = [];
        outDir = fullfile(figDir,[tcDir '_2ndderivative'],roiName);
        if ~exist(outDir,'dir')
            mkdir(outDir)
        end
        ...
        [fig,leg]=plotNiceLinesEBar(1:nTRs-2,mean_tc,se_tc,cols,lspec,pvals,lineLabels,xlab,ylab,figtitle,savePath,plotToScreen);

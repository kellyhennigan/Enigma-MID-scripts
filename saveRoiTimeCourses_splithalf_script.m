...
funcFilePath = fullfile(dataDir, '%s','func_proc','pp_mid%s_tlrc.nii.gz');
...
censorFilePath = fullfile(dataDir,'%s','func_proc','mid%s_censor.1D'); % this is only used if censorBadTRs=1
...
stimFiles = {'gain0_trial_mid%s.1D',...
    'gain1_trial_mid%s.1D',...
    'gain5_trial_mid%s.1D',...
    'gainwin_trial_mid%s.1D',...
    'gainmiss_trial_mid%s.1D',...
    'loss0_trial_mid%s.1D',...
    'loss1_trial_mid%s.1D',...
    'loss5_trial_mid%s.1D',...
    'losswin_trial_mid%s.1D',...
    'lossmiss_trial_mid%s.1D'};


% list corresponding to stims to use in outfile name
outStimNames = {'gain0_half%s','gain1_half%s','gain5_half%s','gainwin_half%s','gainmiss_half%s',...
    'loss0_half%s','loss1_half%s','loss5_half%s','losswin_half%s','lossmiss_half%s'};
...
outDir = fullfile(dataDir,'timecourses_mid_splithalf');
...
for half=1:2
    ...
    for i=1:numel(subjects) % subject loop
        ...
        func = niftiRead(sprintf(funcFilePath,subject,num2str(half)));
        
        
        % if provided, load subject's censor file that says which volumes to
        % censor due to motion
        if censorBadTRs
            censorVols = find(dlmread(sprintf(censorFilePath,subject,num2str(half)))==0);
        end
    ...
    for j=1:numel(rois)
        ...
        for k=1:numel(outStimNames)
            ...
            writetable(T,fullfile(thisOutDir,[sprintf(outStimNames{k},num2str(half)) '.csv']),'WriteVariableNames',0);
        end
    end

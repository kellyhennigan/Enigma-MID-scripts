...
# usage: pre-processing pipeline for MID fMRI data
...
# output files are: 
	...
	# mid_nacc_ts.1D - time series for NAcc VOI 
	# mid_csf_ts.1D - time series for white matter VOI (to be used as a nuisance regressor)
	# mid_wm_ts.1D - time series for cerebrospinal fluid VOI (to be used as a nuisance regressor)
...
######################## create WM, CSF and nacc time series

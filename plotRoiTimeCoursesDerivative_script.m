...
for r = 1:numel(roiNames)
    ...
    for f = 1:nFigs
        ...
        % filepath, if saving
        savePath = [];
        outDir = fullfile(figDir,[tcDir '_derivative'],roiName);
        if ~exist(outDir,'dir')
            mkdir(outDir)
        end
        ...
        [fig,leg]=plotNiceLinesEBar(1:nTRs-1,mean_tc,se_tc,cols,lspec,pvals,lineLabels,xlab,ylab,figtitle,savePath,plotToScreen);
